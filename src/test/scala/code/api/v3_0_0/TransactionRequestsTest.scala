...
class TransactionRequestsTest extends V300ServerSetup with DefaultUsers {
  ...
  def defaultSetup(transactionRequestTypeInput : String= "SANDBOX_TAN") =
    new {
      ...
      def setCreateTransactionRequestType(transactionRequestType: String) = {
        ...
        val createTransReqRequestNew = (v3_0Request / "banks" / testBank.bankId.value / "accounts" / fromAccount.accountId.value /
      ...
      /**
        * Create Transaction Request. -- V300
        */
      var createTransReqRequest = (v3_0Request / "banks" / testBank.bankId.value / "accounts" / fromAccount.accountId.value /
        "owner" / "transaction-request-types" / transactionRequestType / "transaction-requests").POST <@ (user1)
      ...
      /**
        * Get all Transaction Requests. - V300
        */
      var getTransReqRequest = (v3_0Request / "banks" / testBank.bankId.value / "accounts" / fromAccount.accountId.value /
      ...
      /**
        ...
        */
      var getTransactionRequest = (v3_0Request / "banks" / testBank.bankId.value / "accounts" / fromAccount.accountId.value / "owner" / "transactions").GET <@ (user1)
      ...
      /**
        * Answer Transaction Request Challenge - V300
        */

      var answerRequest = (v3_0Request / "banks" / testBank.bankId.value / "accounts" / fromAccount.accountId.value /
        "owner" / "transaction-request-types" / transactionRequestType / "transaction-requests" / transRequestId / "challenge").POST <@ (user1)
  ...
  feature("Security Tests: permissions, roles, views...") {
    ...
    if (Props.getBool("transactionRequests_enabled", false) == false) {
      ...
      scenario("No login user", TransactionRequest) {
        ...
        var request = (v3_0Request / "banks" / helper.fromAccount.bankId.value / "accounts" / helper.fromAccount.accountId.value /
    ...
    if (Props.getBool("transactionRequests_enabled", false) == false) {
      ...
      scenario("No owner view, No CanCreateAnyTransactionRequest role", TransactionRequest) {
        ...
        val request = (v3_0Request / "banks" / helper.testBank.bankId.value / "accounts" / helper.fromAccount.accountId.value /
    ...
    if (Props.getBool("transactionRequests_enabled", false) == false) {
      ...
      scenario("No owner view, With CanCreateAnyTransactionRequest role", TransactionRequest) {
        ...
        var request = (v3_0Request / "banks" / helper.testBank.bankId.value / "accounts" / helper.fromAccount.accountId.value /
    ...
    if (Props.getBool("transactionRequests_enabled", false) == false) {
      ...
      scenario("Invalid transactionRequestType", TransactionRequest) {
        ...
        Then("We call createTransactionRequest with invalid transactionRequestType - V300")
  ...
  feature("we can create transaction requests -- SANDBOX_TAN") {
    ...
    if (Props.getBool("transactionRequests_enabled", false) == false) {
      ...
      scenario("No challenge, No FX (same currencies)", TransactionRequest) {
        ...
        When("we need check the 'Get all Transaction Requests. - V300' to double check it in database")
        ...
        When("we need to check the 'Get Transactions for Account (Full) -V300' to check the transaction info ")

...
object Glossary {
  ...
  |* linkedCustomers is a list of Customers the User is explicitly linked to. See Create User Customer Links endpoint.
  |* userAuthContexts may contain the customer number or other tokens in order to boot strap the User Customer Links
  |or provide an alternative method of tagging the User with an authorisation context. See the Create UserAuthContexts endpoint.
  ...
  |* authViews are entitlements given by account holders to third party users e.g. Sam may grant her accountant Jill read only access to her business account.
  |

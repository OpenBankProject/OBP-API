...
*/
  ...
  */
  ...
  def onlyReturnsDummyData(obpEndpoint: OBPEndpoint) = resourceDocs.find(_.partialFunction == obpEndpoint).map(_.tags.contains(apiTagDummyData)).getOrElse(false)
    
  def dummyDataText(obpEndpoint: OBPEndpoint) = if (onlyReturnsDummyData(obpEndpoint)) "This endpoint currently only returns example data. " else ""

...
//          .accountNumber(otherPartyThisAccountId)
  ...
  override def createCounterparty(createdByUserId: String,
    ...
    val counterpartyId = APIUtil.createOrGetOBPId(s"EXPLICIT" +
      s"$thisBankId$thisAccountId$thisViewId$name$otherAccountRoutingScheme" +
    ...
    val metadata = Counterparties.counterparties.vend.getOrCreateMetadata(BankId(thisBankId), AccountId(thisAccountId), name, counterpartyId, otherAccountRoutingAddress).openOrThrowException("Can not getOrCreateMetadata !")

...
object MapperCounterparties extends Counterparties with MdcLoggable {
  override def getOrCreateMetadata(bankId: String, accountId: String, counterpartyName: String, CounterpartyId: String): Box[CounterpartyMetadata] = {
    ...
    def newPublicAliasName(): String = {
      ...
      val allCounterpartyMetadatas = MappedCounterpartyMetadata.findAll(
        By(MappedCounterpartyMetadata.thisBankId, bankId),
        By(MappedCounterpartyMetadata.thisAccountId, accountId)
      )
      /**
      ...
      def isDuplicate(publicAlias: String) : Boolean = {
        allCounterpartyMetadatas.map(_.getPrivateAlias).contains(publicAlias)
      }
    ...
    def findMappedCounterpartyMetadata(
      ...
      MappedCounterpartyMetadata.find(
        By(MappedCounterpartyMetadata.thisBankId, bankId),
        By(MappedCounterpartyMetadata.thisAccountId, accountId),
        By(MappedCounterpartyMetadata.holder, counterpartyName),
        By(MappedCounterpartyMetadata.accountNumber, otherAccountRoutingAddress))
    }
    
    val existing = findMappedCounterpartyMetadata(bankId, accountId, counterpartyName,CounterpartyId)
    
    existing match {
      case Full(e) => Full(e)
      // Create it!
      case _ => {
        logger.debug(s"Before creating MappedCounterpartyMetadata bankId =$bankId, accountId =$accountId, counterpartyName =$counterpartyName,CounterpartyId =$CounterpartyId")
...
class MappedCounterparty extends CounterpartyTrait with LongKeyedMapper[MappedCounterparty] with IdPK with CreatedUpdated with OneToMany[Long, MappedCounterparty] {
  ...
  object mLabel extends MappedString(this, 36)
  object mKind extends MappedString(this, 36)
  ...
  object mOtherAccountId extends AccountIdString(this)
  ...
  override def label = mLabel.get
  override def kind = mKind.get

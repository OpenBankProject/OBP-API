...
private object LocalRecordConnector extends Connector with MdcLoggable {
  ...
  private def createTransaction(env: OBPEnvelope, theAccount: Account): Transaction = {
    ...
    val otherAccount = new Counterparty(
      ...
      otherAccountRoutingAddress = otherAccount_.bank.get.IBAN.get,
      thisAccountId = otherAccount_.number.get,
      thisBankId = otherAccount_.bank.get.name.get,
      kind = otherAccount_.kind.get,
      otherBankId = theAccount.bankId.value,
      otherAccountId = theAccount.accountId.value,
 ...
 */
  ...
  private def createOtherBankAccount(originalPartyBankId: BankId, originalPartyAccountId: AccountId,
    ...
    new Counterparty(
      ...
      otherBankRoutingAddress = "", 
      otherAccountRoutingAddress = otherAccountFromTransaction.bank.get.IBAN.get,
      thisAccountId = otherAccountFromTransaction.number.get,
      thisBankId = otherAccountFromTransaction.bank.get.name.get,
      kind = "",
      otherBankId = originalPartyBankId.value,
      otherAccountId = originalPartyAccountId.value,

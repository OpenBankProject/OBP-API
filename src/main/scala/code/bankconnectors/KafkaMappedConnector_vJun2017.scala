...
object KafkaMappedConnector_vJun2017 extends Connector with KafkaHelper with MdcLoggable {
  ...
  override def getBanks: List[Bank] = {
    val req = GetBanks(
      AuthInfo(username = currentResourceUserId,
      userId = AuthUser.getCurrentUserUsername), version = messageFormat
    )
    ...
    val rList = {
      cachedBanks.getOrElseUpdate( req.toString, () => process(req))
    }

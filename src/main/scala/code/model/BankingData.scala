...
trait BankAccount extends MdcLoggable {
  ...
  */
  ...
  final def moderatedTransaction(transactionId: TransactionId, view: View, user: Box[User], session: Option[CallContext] = None) : Box[ModeratedTransaction] = {
    if(APIUtil.hasAccess(view, user))
      Connector.connector.vend.getTransaction(bankId, accountId, transactionId, session).flatMap(view.moderate)
    else
      viewNotAllowed(view)
  }
  ...
  */
  ...
  final def getModeratedTransactions(user : Box[User], view : View, session: Option[CallContext], queryParams: OBPQueryParam* ): Box[List[ModeratedTransaction]] = {
    if(APIUtil.hasAccess(view, user)) {
      for {
  ...
  // TODO We should extract params (and their defaults) prior to this call, so this whole function can be cached.
  final def getModeratedTransactionsCore(user : Box[User], view : View, session: Option[CallContext] , queryParams: OBPQueryParam* ): Box[List[ModeratedTransactionCore]] = {
    if(APIUtil.hasAccess(view, user)) {
      for {

...
class KafkaTransaction extends TransactionUUID {

  var _bank = ""
  var _account = ""
  var _counterpartyIban = ""

  def counterpartyIban: String ={ _counterpartyIban }

  private val logger = Logger(classOf[KafkaTransaction])

  def bank: String = _bank
  def account: String = _account
  ...
  def toTransaction(account: BankAccount): Option[Transaction] = {
   /* val tBankId = theBankId
    ...
    if (tBankId != account.bankId || tAccId != account.accountId) {
      logger.warn("Attempted to convert KafkaTransaction to Transaction using unrelated existing BankAccount object")
      None
    } else {
      val label = {
...
object KafkaTransaction extends KafkaTransaction {
  /*
  private val logger = Logger(classOf[KafkaTransaction])

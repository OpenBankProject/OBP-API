    ...
    if (tBankId != account.bankId || tAccId != account.accountId) {
      ...
      def createCounterparty(alreadyFoundMetadata : Option[CounterpartyMetadata]) = {
        new Counterparty(
          ...
          otherAccountRoutingAddress = getCounterpartyIban().getOrElse(""),
          thisAccountId = counterpartyAccountNumber.get,
          thisBankId = counterpartyBankName.get,
          kind = counterpartyAccountKind.get,
          otherBankId = theBankId.value,
          otherAccountId = theAccountId.value,

...
RUN --mount=type=cache,target=$HOME/.m2 MAVEN_OPTS="-Xmx8G -Xss4m" mvn install -pl .,obp-commons
RUN --mount=type=cache,target=$HOME/.m2 MAVEN_OPTS="-Xmx8G -Xss4m" mvn install -DskipTests -pl obp-api

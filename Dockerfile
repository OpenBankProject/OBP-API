...
RUN --mount=type=cache,target=$HOME/.m2 MAVEN_OPTS="-Xmx12G -Xss1G" mvn clean install -pl .,obp-commons -X
RUN --mount=type=cache,target=$HOME/.m2 MAVEN_OPTS="-Xmx12G -Xss1G" mvn clean install -DskipTests -pl obp-api -X

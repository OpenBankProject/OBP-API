...
object MappedTransactionRequestProvider extends TransactionRequestProvider with MdcLoggable {
  ...
  override def createTransactionRequestImpl210(transactionRequestId: TransactionRequestId,
    ...
    // The attribution call (UserReference.TransactionRequestUserIdOnBehalfOfUserId) answers the second; the request
    ...
    val transactionRequestAttribution: Option[code.users.Attribution] = authenticatedUserId.flatMap(userId =>
      code.users.Users.users.vend.attributionOf(userId, code.users.UserReference.TransactionRequestUserIdOnBehalfOfUserId).toOption)

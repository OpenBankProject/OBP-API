...
class MappedTransactionRequest extends LongKeyedMapper[MappedTransactionRequest] with IdPK with CreatedUpdated with CustomJsonFormats {
  ...
  def toTransactionRequest : Option[TransactionRequest] = {
    ...
    val t_to_sandbox_tan = if (TransactionRequestTypes.withName(transactionType) == TransactionRequestTypes.SANDBOX_TAN || TransactionRequestTypes.withName(transactionType) == TransactionRequestTypes.ACCOUNT)

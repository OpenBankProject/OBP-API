...
trait AtmsProvider extends MdcLoggable {
  ...
  // Widened so LocalMappedConnector can reach the atm table through the provider instead of
  // through MappedAtm directly — the prerequisite for deleting the entity.

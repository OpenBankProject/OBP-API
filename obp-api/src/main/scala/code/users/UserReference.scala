...
object UserReference {
  ...
  case object AccountWebhook_CreatedByUserId                   extends UserReference(UseOnBehalfOfUserId   , "code.webhook.MappedAccountWebhook", List("mCreatedByUserId"), "ownership KEY, not just provenance: getAccountWebhooksByUserIdFuture queries By(mCreatedByUserId, userId), so an agent-created webhook is invisible to the human")

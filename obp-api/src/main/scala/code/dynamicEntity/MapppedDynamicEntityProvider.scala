    ...
    tryo{
      try {
        val saved = entityToPersist
          ...
          // Definition creator resolves to the on-behalf-of user (UserReference.DynamicEntityUserId):
          // a consent user owns nothing durable. ON_BEHALF_OF_USER_ID_PLAN.md, Phase 2.
          .UserId(code.users.Users.users.vend.attributedUserId(dynamicEntity.userId, code.users.UserReference.DynamicEntityUserId).openOr(dynamicEntity.userId))

...
Returns the content of a payment object""",
  ...
  lazy val getInstantPaymentInformation: OBPEndpoint = {
    case "payments" :: paymentProduct :: paymentId :: Nil JsonGet _ => {
      cc =>
        // Вызов метода для получения информации о платёжной транзакции
  ...
  def getInstantPaymentInformationImplementation(paymentProduct: String, paymentId: String, getStatus: Boolean, cc: CallContext) = {
    for {
      ...
      // --- Вызов коннектора для получения транзакции по ID для INSTANT_CREDIT_TRANSFERS_MD ---
...
Check the transaction status of a payment initiation.""",
  ...
  def initiatePaymentImplementation(paymentService: String, paymentProduct: String, json: liftweb.json.JValue, cc: CallContext) = {
    for {
      ...
      transferRequest <- {
        if (paymentServiceType == PaymentServiceTypes.payments) transactionRequestType match {
          case TransactionRequestTypes.SEPA_CREDIT_TRANSFERS =>

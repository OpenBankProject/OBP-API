...
object MigrationOfMappedPayment {

  val oneDayAgo = ZonedDateTime.now(ZoneId.of("UTC")).minusDays(1)
  val oneYearInFuture = ZonedDateTime.now(ZoneId.of("UTC")).plusYears(1)
  val formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm'Z'")

  def alterColumnPaymentId(name: String): Boolean = {
          ...
          DbFunction.maybeWrite(true, Schemifier.infoF _) {
            APIUtil.getPropsValue("db.driver") match {
              case Full(dbDriver) if dbDriver.contains("com.microsoft.sqlserver.jdbc.SQLServerDriver") =>
                () =>
                  """ALTER TABLE mappedpayment ALTER COLUMN mpaymentid varchar(40);"""
              case _ =>
                () =>
                  """ALTER TABLE mappedpayment ALTER COLUMN mpaymentid type varchar(40);"""
            }
          }
        ...
        val executedSql =
          DbFunction.maybeWrite(true, Schemifier.infoF _) {
            APIUtil.getPropsValue("db.driver") match {
              case Full(dbDriver) if dbDriver.contains("com.microsoft.sqlserver.jdbc.SQLServerDriver") =>
                () =>
                  """ALTER TABLE mappedpayment ALTER COLUMN mremittanceinformationunstructured varchar(200);"""
              case _ =>
                () =>
                  """ALTER TABLE mappedpayment ALTER COLUMN mremittanceinformationunstructured type varchar(200);"""
            }
          }

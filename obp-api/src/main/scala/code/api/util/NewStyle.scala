...
object NewStyle extends MdcLoggable{
  ...
  object function {
    ...
    def updateConsumer(id: Long, 
      ...
      Future(Consumers.consumers.vend.updateConsumer(id, key, secret, isActive, name, alias, appType, company, description, developerEmail, redirectURL, createdByUserId, logoURL, certificate)) map {
    ...
    def createInstantTransactionRequestMdBGV1(
      ...
      response.flatMap { i =>
        ...
        Helper.booleanToFuture(
          ...
          trx.errorCode.isEmpty   // успех только если errorCode пустой
        ).map(_ => (trx, i._2))
      }
    }

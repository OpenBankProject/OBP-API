...
class ImporterTest extends ServerSetup with MdcLoggable with DefaultConnectorTestSetup {
  ...
  feature("Importing transactions via an API call") {
    ...
    scenario("Attempting to import transactions with the correct secret key") {
      ...
      val account = Connector.connector.vend.getBankAccountLegacy(f.account.bankId, f.account.accountId, None).map(_._1).openOrThrowException(attemptedToOpenAnEmptyBox)
    ...
    scenario("Adding transactions that have already been imported") {
      ...
      var account = Connector.connector.vend.getBankAccountLegacy(f.account.bankId, f.account.accountId, None).map(_._1).openOrThrowException(attemptedToOpenAnEmptyBox)
      ...
      account = Connector.connector.vend.getBankAccountLegacy(f.account.bankId, f.account.accountId, None).map(_._1).openOrThrowException(attemptedToOpenAnEmptyBox)

...
class DynamicMessageDocTest extends V400ServerSetup {
  ...
  feature("Test the DynamicMessageDoc endpoints") {
    scenario("We create my DynamicMessageDoc and get,update", ApiEndpoint1,ApiEndpoint2, ApiEndpoint3, ApiEndpoint4, VersionOfApi) {
      ...
      (dynamicMessageDocs \ "example_outbound_message").values.toString should equal (postDynamicMessageDoc.exampleOutboundMessage)
      (dynamicMessageDocs \ "example_inbound_message").values.toString should equal (postDynamicMessageDoc.exampleInboundMessage)

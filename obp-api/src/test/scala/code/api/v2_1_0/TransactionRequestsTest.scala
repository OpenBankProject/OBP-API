...
class TransactionRequestsTest extends V210ServerSetup with DefaultUsers {
  ...
  class DefaultSetup(transactionRequestTypeInput : String= SANDBOX_TAN.toString) {
      ...
      def makeGetTransReqRequest = makeGetRequest(getTransReqRequest, List(("from_date", APIUtil.oneYearAgoFromDateString),("to_date", APIUtil.DefaultToDateString)))
      ...
      def makeGetTransRequest = makeGetRequest(getTransactionRequest, List(("from_date", APIUtil.oneYearAgoFromDateString),("to_date", APIUtil.DefaultToDateString)))

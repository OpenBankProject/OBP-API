...
class DisposableDatabaseGuardTest extends AnyFlatSpec with Matchers {
  ...
  "the guard" should "allow an in-memory H2 database" in {
    ...
    isDisposable("jdbc:h2:mem:flywayBaselineExisting;DB_CLOSE_DELAY=-1") should equal(true)
  }
  ...
  it should "allow the per-shard and migration-test Postgres databases" in {
    ...
    isDisposable("jdbc:postgresql://localhost:5432/obp_flyway_migration_test") should equal(true)
  ...
  it should "refuse a name that merely starts like an allowed one" in {
    ...
    isDisposable("jdbc:postgresql://localhost:5432/obp_flyway_migration_test_real") should
      equal(false)

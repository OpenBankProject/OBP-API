...
import code.api.Constant.{SYSTEM_ACCOUNTANT_VIEW_ID, SYSTEM_AUDITOR_VIEW_ID, SYSTEM_ENABLE_CUSTOM_VIEWS_VIEW_ID, SYSTEM_FIREHOSE_VIEW_ID, SYSTEM_OWNER_VIEW_ID}
...
trait TestConnectorSetup {
  ...
  final protected def createAccountRelevantResources(user: User, banks : Traversable[Bank]) : Traversable[BankAccount] = {
    ...
    val enableCustomViews = getOrCreateSystemView(SYSTEM_ENABLE_CUSTOM_VIEWS_VIEW_ID)
    ...
    accounts.foreach(account => {
      ...
      Views.views.vend.grantAccessToSystemView(account.bankId, account.accountId, enableCustomViews, user)
